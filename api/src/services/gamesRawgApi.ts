...
export async function getGamesApi(
...
): Promise<Array<VideogamesResults>> {
	...
	if (!Boolean(name)) {
		...
		// RESOLVE PROMISES
		await Promise.all([nexts, nexts1, nexts2]).then((response) => {
	...
	promisesResultApi.forEach((data): void => {
		let platforms: Array<NormalEntity> = data.platforms.map((pl) => {
			return { name: pl.platform.name, id: pl.platform.id };
		});
		let genres: Array<NormalEntity> = data.genres.map((gr) => {
